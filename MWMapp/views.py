...
#     html  = render_to_string('worker_fitnessview.html', { 'pagesize' : 'A4', }, context_instance=RequestContext(request))
...
def updateworker(request):
    ...
    return render(request, 'workersignup_updateworker.html', context=mydict)
...
def updatejb(request):
    ...
    return render(request, 'workersignup_updatejb.html', context=mydict)
...
def reginsurancepage(request):
    ...
    if request.method == 'POST':
        ...
        if userForm.is_valid() and customerForm.is_valid():
            ...
            my_worker_group[0].user_set.add(user)
        return redirect('workerlogin')
    return render(request, 'workersignup_insur.html', context=mydict)
...
def afterlogin_view(request):
    if is_police(request.user):
        ...
        accountapproval = models.Worker.objects.all().filter(
        ...
        accountapproval = models.Jb.objects.all().filter(
            user_id=request.user.id, status=True)
        if accountapproval:
            return redirect('jbdash')
        else:
        ...
        accountapproval = models.Insurance.objects.all().filter(
            user_id=request.user.id, status=True)
        if accountapproval:
            return redirect('insurance_dash')
        else:
...
def insuranceapplyview(request):
    patients = models.Patient1.objects.all().filter(
        status=False, assignedDoctorId=request.user.id)
    return render(request, 'jobapplyview.html', {'patients': patients})
...
def viewprofile(request):
    # those whose approval are needed
    doctors = models.Worker.objects.all().filter(user=request.user.id)
    return render(request, 'workerviewprof.html', {'doctors': doctors})


def viewprofile2(request):
    # those whose approval are needed
    doctors = models.Jb.objects.all().filter(user=request.user.id)
    return render(request, 'workerviewprof2.html', {'doctors': doctors})


def viewprofile3(request):
    # those whose approval are needed
    doctors = models.Insurance.objects.all().filter(user=request.user.id)
    return render(request, 'workerviewprof3.html', {'doctors': doctors})
...
def jb_addjob(request):
    jbp = Jb.objects.get(user__id=request.user.id)
    ...
    if request.method == "POST":
        ...
        jobs(jobprovider=jbp, jbtitle=jbtitle, jbplace=jbplace,
             jbdate=jbdate, jbdes=jbdes, jbname=jbname, jbno=jbno).save()
...
def jb_apply(request):
    if request.method == "POST":
        ...
        applyjobs(jbtitle=jbtitle, jbplace=jbplace, jbdate=jbdate,
                  jbdes=jbdes, jbname=jbname, jbno=jbno).save()
...
def jb_apply1(request):
    if request.method == "POST":
        ...
        insurance_scheme(policyno=policyno, insurancetype=insurancetype, company=company, timelength=timelength,
...
def add_scheme(request):
    # inp = Insurance.objects.get(user__id=request.user.id)
...
def jbcomplaint(request):
    if request.method == "POST":
        ...
        complaintjb(complaintsub=complaintsub,
                    complaintdes=complaintdes).save()
...
def regjobprovider(request):
    if request.method == "POST":
        ...
        jobprovider(user_name=user_name, email=email, dob=dob, gender=gender,
                    photoid=photoid, no=no, password=password, password2=password2).save()
...
def reginsurance(request):
    if request.method == "POST":
        ...
        insuranceagency(regno=regno, name=name, email=email, type1=type1,
                        no=no, password=password, password2=password2).save()
...
def worker_viewappliedjob(request): 
    job_view = Patient.objects.all().filter(user__id=request.user.id)
...
def worker_complaint(request):
    if request.method == "POST":
        ...
        complaintworker(complaintsub=complaintsub,
                        complaintdes=complaintdes).save()
...
def worker_viewappliedpolicy(request):
    job_view = Patient1.objects.all().filter(
...
def worker_viewpolicy(request):
    ...
    insurance_view = Insurance.objects.all()
    
    return render(request, 'worker_viewpolicy.html', {'insurance_view': insurance_view})


def jb_viewaddedjob(request):
    jbprovider = Jb.objects.get(user__id=request.user.id)
    jbs = []

    print(jbs)
    job_view = jobs.objects.filter(
        jobprovider__id=jbprovider.id)
    return render(request, 'jb_viewaddedjob.html', {'job_view': job_view})


def jb_viewaddedjob1(request):
    # job_view = insurance_scheme.objects.filter(user=request.user.id)
    job_view = insurance_scheme.objects.filter()
...
def worker_viewcomp(request):
    job_view = complaintworker.objects.all()
    return render(request, 'complaintsub.html', {'job_view': job_view})


def worker_viewcompjb(request):
    job_view = complaintjb.objects.all()
    return render(request, 'complaintsubJB.html', {'job_view': job_view})


def worker_viewcompin(request):
    job_view = complaintin.objects.all()
    return render(request, 'complaintsubin.html', {'job_view': job_view})


def workerview(request):
    job_view = Worker.objects.all()
    return render(request, 'adminworkerview.html', {'job_view': job_view})


def jbviewworkers(request):
    job_view = Worker.objects.all()
    return render(request, 'jbworkerview.html', {'job_view': job_view})




def jbview(request):
    job_view = Jb.objects.all()
    return render(request, 'adminjbview.html', {'job_view': job_view})


def inview(request):
    job_view = Insurance.objects.all()
    return render(request, 'admininview.html', {'job_view': job_view})


def workerviewreport(request):
    approve = models.Product.objects.all().filter(user__id=request.user.id)
    return render(request, 'worker_fitnessview.html', {'approve': approve})
...
def addProduct(request):
    ...
    if request.method == 'POST':
        print("hii")
        user = request.user.id
        form = ProductForm(request.POST, request.FILES, user)
...
def appliedjob_view(request):
    ...
    patientcount = models.worker.objects.all().filter(
